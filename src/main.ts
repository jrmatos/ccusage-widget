...
const appName =
  packageJson.build?.productName || packageJson.name || "CCUsage Widget";
app.setName(appName);
...
async function fetchAndUpdateTrayTitle() {
  ...
  try {
    ...
    // Find today's data - if not found, show $0.00
    const today = dailyArray.find((d: any) => d.date === todayDate);

    if (today) {
      const cost = today.totalCost || today.costUSD || today.totalCostUSD || 0;
      tray.setTitle(`Claude Code: $${cost.toFixed(2)}`);
    } else {
      // If today's data doesn't exist yet, show $0.00 (don't fall back to yesterday)
    ...
    authors: packageJson.author
      ? [
          typeof packageJson.author === "string"
            ? packageJson.author
            : packageJson.author.name || "JeongJaeSoon",
        ]
      : ["JeongJaeSoon"],
    website:
      packageJson.homepage || "https://github.com/JeongJaeSoon/ccusage-widget",
  });
...
ipcMain.handle("get-usage-data", async () => {
  try {
    ...
    try {
      // First try with npx
      [dailyResult, monthlyResult, sessionResult, blocksResult] =
        await Promise.all([
    ...
    const thisMonth =
      monthlyArray.find((m: any) => m.month === thisMonthDate) ||
      monthlyArray[0] ||
      null;
    ...
    const recentSessions = sessionsArray.slice(0, 5).map((s: any) => {
      ...
      let sessionName =
        s.sessionName || s.session || s.name || "Unknown Session";
      ...
      return {
        ...
        lastActivity:
          s.lastActivity || s.lastUpdated || new Date().toISOString(),
      };
    });

    // Get blocks data
    const blocksArray = Array.isArray(blocksData.blocks)
      ? blocksData.blocks
      : [];

    // Find current active block
    const currentBlock = blocksArray.find(
      (block: any) =>
    ...
    const result = {
      ...
      currentBlock: currentBlock
        ? {
            tokens:
              (currentBlock.tokenCounts?.inputTokens || 0) +
              (currentBlock.tokenCounts?.outputTokens || 0),
